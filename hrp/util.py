...
def log_call(level=logging.DEBUG):
    """
    logs callable function and arguments
    ...
    def decorator(fun):
        ...
        def inner(*args, **kwargs):
            """
            inner execution, logs function name and arguments
            Parameters
            ----------
            ...
            **kwargs : named arguments
            ...
            res = inspect.getcallargs(fun, *args, **kwargs)
            #print res debug
            LOGGER.log(level,
                       "CALL hrp.%s(%s)", fun.__name__,
                       ", ".join(
                           ["{}={}".format(k, res[k]) for k in res if k != 'self']))
...
if sys.version_info[0] == 2:
    ...
    def _ord(byt):
        """
        convert byte to int (dummy for python3)
        Parameters
        ----------
        byt : byte character
...
def crc16(data, crc=0x0000):
    """CRC-16-BUYDATA

    Parameters
    ----------
    data :
        bytearray to calculate
    crc :
